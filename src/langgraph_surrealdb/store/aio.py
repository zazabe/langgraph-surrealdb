...
class AsyncSurrealStore(AsyncBatchedBaseStore):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        table = settings.store_table
        self.store_factory = DbStoreModelFactory(table=table)
        self.vector_factory = DbStoreVectorModelFactory(table=table)
    ...
    async def start_ttl_sweeper(
        ...
        interval = float(
            sweep_interval_minutes or self.settings.ttl.sweep_interval_minutes
            if self.settings.ttl.sweep_interval_minutes is not None
            else 5
        )
        ...
        async def sweep_loop() -> None:
            while not self._ttl_stop_event.is_set():
                try:
                    ...
                    try:
                        count = await self.sweep_ttl()
                        if count:
                            logger.info("Store swept %d expired items", count)
                    except Exception:
                        logger.exception("Store TTL sweep iteration failed")

        self._ttl_sweeper_task = asyncio.create_task(
            sweep_loop(), name="surreal-store-ttl-sweeper"
        )
        return self._ttl_sweeper_task
    ...
    def _is_ttl_enabled(self) -> bool:
        return self.settings.ttl.default_ttl is not None
